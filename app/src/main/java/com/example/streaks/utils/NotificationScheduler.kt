package com.example.streaks.utils

import android.content.Context
import androidx.work.*
import com.example.streaks.data.Reminder
import com.example.streaks.workers.ReminderWorker
...
class NotificationScheduler(private val context: Context) {
    ...
    private fun scheduleDailyReminder(streakId: String, streakName: String, time: LocalTime, reminder: Reminder) {
        ...
        val delayMillis = triggerTime.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli() - 
                         System.currentTimeMillis()

        val inputData = Data.Builder()
            .putString(ReminderWorker.STREAK_ID_KEY, streakId)
            .putString(ReminderWorker.STREAK_NAME_KEY, streakName)
            .putString(ReminderWorker.REMINDER_TEXT_KEY, "Time to work on your $streakName streak!")
            .build()
    ...
    private fun scheduleWeeklyReminder(streakId: String, streakName: String, time: LocalTime, dayOfWeek: Int, reminder: Reminder) {
        val now = LocalDateTime.now()
        ...
        val delayMillis = triggerDate.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli() - 
                         System.currentTimeMillis()
    ...
    fun cancelReminder(streakId: String) {
        ...
        WorkManager.getInstance(context).cancelAllWorkByTag("${REMINDER_WORK_PREFIX}${streakId}_daily")
        
        // Cancel weekly reminders for all days
        for (day in 0..6) {
            WorkManager.getInstance(context).cancelAllWorkByTag("${REMINDER_WORK_PREFIX}${streakId}_$day")
        }
    }
